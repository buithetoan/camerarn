...
class Camera extends Component{
        ...
        <KeyboardShiftView style={styles.content} keyboardShouldPersistTaps={'never'} extraHeight={0} bounces={false}>
          ...
          <View style={mainViewStyle}>
            ...
            >
                  ...
                  : null}
                  { this.state.imageUri ? this.renderSmallImage(this.state.imageUri) : null }
            </RNCamera>
  ...
  renderSmallImage(url){  
    return(
      <TouchableOpacity onPress={() => this.props.navigation.navigate("Show",{url: url})} style={styles.smallImage}>
  ...
  takePicture = async () => {
    if (this.camera) {
      ...
      if(data.uri){
        this.savePicture(data.uri);
      }
      // Alert.alert("Picture Taken!", JSON.stringify(data, null, 2));

    }
  }
  savePicture = async function(url) {
    CameraRoll.save(url,'photo')
      .then(
        this.setState({imageUri: url})
      );
  }; 
  saveVideo = async function (url) {
    CameraRoll.save(url,'video');
  }
  startVideo = async () => {
